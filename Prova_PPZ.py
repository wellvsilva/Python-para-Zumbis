...
def squirrel_play(temp, is_summer):
  play = 10 if is_summer else 0
  if temp >= 60 and temp <= 90 + play:
    return True
  return False
...
def pego_correndo(speed, is_birthday):
  pego = 5 if is_birthday else 0
  if speed <= 60 + pego:
      return 0
  elif speed > 60 + pego and speed <= 80 + pego:
    return 1
  else:
    return 2
...
def fila_tijolos(n_peq, n_gra, meta):
  ...
  total_gra = n_gra
  restante = meta % 5    
  fila = (meta - restante) // 5
  if(fila > total_gra):
       med = fila - total_gra
       total_grandes = 0           
       meta_total += (med * 5) + restante 
  else:
      total_gra +=  - fila 
      meta_total = restante
      if meta_total == 0:
          return True
  total_peq +=  - meta_total
  if total_peq >= 0:           
      return True
  else:
      return False   
